...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Beta6 AE feature experiments.")
    parser.add_argument("--config", default="configs/01_main_beta6_ae_features.yaml", help="Path to YAML config.")
...
def train_autoencoder_early_stop(x_train: np.ndarray, cfg: dict[str, Any]) -> tuple[Any, Any, int, pd.DataFrame]:
    ...
    patience_left = int(cfg.get("patience", 8))
    best_loss = np.inf
    best_epoch = 0
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        model.eval()
        with torch.no_grad():
            valid_loss = float(mse(model(valid_tensor), valid_tensor).detach().cpu())
        train_loss = train_loss_sum / max(train_count, 1)
        rows.append({"epoch": epoch, "train_recon_mse": train_loss, "valid_recon_mse": valid_loss})
...
def train_autoencoder_fixed_epochs(x_train: np.ndarray, cfg: dict[str, Any], epochs: int) -> tuple[Any, Any]:
    torch = setup_torch(cfg)
    seed_everything(torch, int(cfg.get("seed", 2026)))
    device = torch.device("cpu")
    network = AutoEncoder(
...
def main() -> int:
    ...
    if bool(input_cfg.get("include_interactions", True)):
        x_int_train = transform_interactions(x_base_train, interaction_defs, eps=1e-6)
        x_int_valid = transform_interactions(x_base_valid, interaction_defs, eps=1e-6)
        x_int_full = transform_interactions(x_base_full, interaction_defs, eps=1e-6)
        x_int_test = transform_interactions(x_base_test, interaction_defs, eps=1e-6)
        matrices_train.append(x_int_train)
        matrices_valid.append(x_int_valid)
        matrices_full.append(x_int_full)
        matrices_test.append(x_int_test)
    ...
    if shap_valid is None:
        ...
        shap_valid, _, _ = train_xgb_model(
    ...
    print(f"Training AE on {x_ae_train.shape[0]} rows and {x_ae_train.shape[1]} features.")
    torch, ae_network, best_epoch, ae_log = train_autoencoder_early_stop(x_ae_train, ae_cfg)
    ae_log.to_csv(run_dir / "ae_training_log.csv", index=False)
    z_train = encode_autoencoder(torch, ae_network, x_ae_train, int(ae_cfg["batch_size"]))
    z_valid = encode_autoencoder(torch, ae_network, x_ae_valid, int(ae_cfg["batch_size"]))
    del ae_network, x_ae_train, x_ae_valid
    gc.collect()
    ...
    torch_full, ae_full_network = train_autoencoder_fixed_epochs(x_ae_full, ae_cfg, best_epoch)
    z_full = encode_autoencoder(torch_full, ae_full_network, x_ae_full, int(ae_cfg["batch_size"]))
    z_test = encode_autoencoder(torch_full, ae_full_network, x_ae_test, int(ae_cfg["batch_size"]))
    del ae_full_network, x_ae_full, x_ae_test
    gc.collect()

    ae_feature_names = [f"ae_{idx}" for idx in range(z_train.shape[1])]
    write_selected_features(run_dir / "ae_feature_names.txt", ae_feature_names)
    pd.DataFrame(z_valid, columns=ae_feature_names).to_csv(run_dir / "valid_ae_features.csv", index=False)

    x_input_ae_train = np.hstack([x_input_train_raw, z_train]).astype(np.float32, copy=False)
    x_input_ae_valid = np.hstack([x_input_valid_raw, z_valid]).astype(np.float32, copy=False)
    x_input_ae_full = np.hstack([x_input_full_raw, z_full]).astype(np.float32, copy=False)
    x_input_ae_test = np.hstack([x_input_test_raw, z_test]).astype(np.float32, copy=False)
    signal_matrices = {
        "ae_ridge": (z_train, z_valid, z_full, z_test),
        "input_plus_ae_ridge": (x_input_ae_train, x_input_ae_valid, x_input_ae_full, x_input_ae_test),
        "input_plus_ae_xgboost": (x_input_ae_train, x_input_ae_valid, x_input_ae_full, x_input_ae_test),
    }

    metric_rows: list[dict[str, Any]] = []
    candidate_rows: list[dict[str, Any]] = []
    for signal_name in [str(name) for name in config["signals"]]:
        print(f"Training downstream signal {signal_name}.")
        x_train_signal, x_valid_signal, x_full_signal, x_test_signal = signal_matrices[signal_name]
        if signal_name.endswith("_ridge"):
            train_scaler = MatrixStandardizer()
            x_train_scaled = train_scaler.fit_transform(x_train_signal)
            x_valid_scaled = train_scaler.transform(x_valid_signal)
            full_signal_scaler = MatrixStandardizer()
            x_full_scaled = full_signal_scaler.fit_transform(x_full_signal)
            x_test_scaled = full_signal_scaler.transform(x_test_signal)
            ...
            ridge_grid.to_csv(run_dir / f"ridge_alpha_search_{signal_name}.csv", index=False)
            test_pred = fit_ridge(float(best["alpha"]), x_full_scaled, y_full, x_test_scaled)
            extra = {"alpha": float(best["alpha"])}
        else:
            train_scaler = MatrixStandardizer()
            x_train_scaled = train_scaler.fit_transform(x_train_signal)
            x_valid_scaled = train_scaler.transform(x_valid_signal)
            ...
            x_full_scaled = full_signal_scaler.fit_transform(x_full_signal)
            x_test_scaled = full_signal_scaler.transform(x_test_signal)
            valid_pred, final_params, xgb_row = train_xgb_model(
                x_train_scaled,
                y_train,
                x_valid_scaled,
                y_valid,
            ...
            test_pred = fit_xgb_predict(final_params, x_full_scaled, y_full, x_test_scaled)
            extra = {
                "best_iteration": xgb_row["best_iteration"],
                "params": json.dumps(final_params, sort_keys=True),
            }
        save_valid_prediction(pred_dir / f"valid_{signal_name}.csv", y_valid, valid_pred)
        signal_submission = make_submission(sample_submission, test_df, test_pred, prediction_col, id_col)
        signal_submission.to_csv(signal_submission_dir / f"submission_{signal_name}.csv", index=False)
        signal_pearson = pearson_corr(y_valid, valid_pred)
        metric_rows.append(
            {
                "model": signal_name,
                "input_feature_count": x_train_signal.shape[1],
                "ae_feature_count": z_train.shape[1],
    ...
    write_json(
        run_dir / "final_summary.json",
        {
            ...
            "ae_bottleneck_dim": int(ae_cfg["bottleneck_dim"]),
